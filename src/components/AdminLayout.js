...
export function AdminLayout({ children }) {
  ...
  const ToolBarCenterContent = (
    <div className='flex align-items-center'>
      ...
      <div className='ml-3 text-xl'>Панель управления {router.route === '/admin' && <span> / Главная</span>}{router.route === '/admin-hotels' && <span> / Отели</span>}{router.route === '/admin-reviews' && <span> / Отзывы</span>}</div>
    </div>
  )
